...
public class Section1 extends Section {
	...
	public void setVerPO(String verPO) {
		this.verPO = verPO;
	}
	...
	public class FieldDictionaryEntry {
		...
		FieldDictionaryEntry(String name, int tag, int requirement,
				int multiplicity, int maximumLength, int reasonableLength,
				String className) {
			this.name = name;
			this.tag = tag;
	...
	private HashMap dictionaryFieldByName; // map of String names to dictionary
											// entries
	private FieldDictionaryEntry[] dictionaryFieldByTag; // (mostly sparse)
															// array of 256
															// possible tags
	...
	private class Field {
		...
		String validate() {
			...
			if (isTerminator() && length != 0) {
				strbuf.append("Terminator should be zero length but has length "
						+ length
						+ " dec (0x"
		...
		public String getValueAsString() {
			return value == null ? ""
					: makeStringFromByteArrayRemovingAnyNulls(value);
		}
	}
	...
	private class TextField extends Field {
		...
		protected int getValueLengthForPurposeOfValidation() {
			...
			if (valueLength != 0 && value[valueLength - 1] == 0) {
				--valueLength; // do not include the trailing null terminator in
								// length
			}
			return valueLength;
		}
	...
	private class TimeZoneField extends Field {
		...
		String validate() {
			...
			if (offset != 0x7fff && (offset < -780 || offset > 780)) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": if specified (not 0x7fff), timezone offset must be +/- 780 minutes (13 hours), got "
						+ offset
						+ " dec (0x"
						+ Integer.toHexString(offset & 0xffff) + ")");
				strbuf.append("\n");
			}
			if (offset != 0x7fff && index != 0) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": index must not be used if timezone offset is used (not 0x7fff), got index of "
						+ index + " dec");
				strbuf.append("\n");
			}
	...
	private static String[] ageUnitsDescription = { "Unspecified", "Years",
			"Months", "Weeks", "Days", "Hours" };
	private static String[] heightUnitsDescription = { "Unspecified",
	...
	private class ValueWithUnitsField extends Field {
		...
		String validate() {
			StringBuffer strbuf = new StringBuffer();
			strbuf.append(super.validate());
			if (units >= unitDescriptors.length
					|| unitDescriptors[units] == null) {
				strbuf.append(describeFieldBriefly(tag));
			...
			if (units == 0 && bvalue != 0) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": units are unspecified, but value is not zero ("
						+ bvalue + " dec)");
				strbuf.append("\n");
			}
		...
		public String getValueAsString() {
			...
			strbuf.append(" ");
			strbuf.append(units < unitDescriptors.length ? unitDescriptors[units]
					: "Unrecognized");
			return strbuf.toString();
		}
	}

	private static String[] sexDescriptors = { "Неизвестно", "Мужской",
			"Женский", null, null, null, null, null, null, "Unspecified" };
	private static String[] raceDescriptors = { "Неизвестно", "Европеоидная",
			"Афроамериканская", "Нордическая" };
	private static String[] statCodeDescriptors = { "Routine", "Emergency 1",
			"Emergency 2", "Emergency 3", "Emergency 4", "Emergency 5",
	...
	private class SingleCodedValueField extends Field {
		...
		public String getValueAsString() {
			...
			strbuf.append(code);
			strbuf.append(" [");
			strbuf.append(code < descriptors.length ? descriptors[code]
					: "Unrecognized");
			strbuf.append("]");
			return strbuf.toString();
		}
	...
	private static String[] electrodePlacement12LeadDescriptors = {
			"Unspecified", "Standard", "Mason-Likar Individual",
			"Mason-Likar One Pad", "All One Pad", "Derived from Frank XYZ",
			"Non-standard" };

	private static String[] electrodePlacementXYZLeadDescriptors = {
			"Unspecified", "Frank", "McFee-Parungao", "Cube",
	...
	private class TwinCodedValueField extends Field {
		...
		TwinCodedValueField(int tag, int length, String descriptors1[],
				String descriptors2[]) {
			this.tag = tag;
			this.length = length;
		...
		String validate() {
			...
			if (code1 >= descriptors1.length || descriptors1[code1] == null) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": Unrecognized first code value " + code1
						+ " dec");
				strbuf.append("\n");
			}
			if (code2 >= descriptors2.length || descriptors2[code2] == null) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": Unrecognized second code value " + code2
						+ " dec");
				strbuf.append("\n");
			}
			return strbuf.toString();
		...
		public String getValueAsString() {
			...
			strbuf.append(code1);
			strbuf.append(" [");
			strbuf.append(code1 < descriptors1.length ? descriptors1[code1]
					: "Unrecognized");
			strbuf.append("], ");
			strbuf.append(code2);
			strbuf.append(" [");
			strbuf.append(code2 < descriptors2.length ? descriptors2[code2]
					: "Unrecognized");
			strbuf.append("]");
			return strbuf.toString();
		}
	...
	private class DateField extends Field {
		...
		String validate() {
			StringBuffer strbuf = new StringBuffer();
			strbuf.append(super.validate());
			if (dd != 0 || mm != 0 || yyyy != 0) { // else is unspecified, which
													// is OK
				if (mm < 1 || mm > 12) {
					strbuf.append(describeFieldBriefly(tag));
		...
		public String toString() {
			...
			strbuf.append(dd);
			strbuf.append(">");
			strbuf.append((dd != 0 || mm != 0 || yyyy != 0) ? ""
					: " Unspecified");
			return strbuf.toString();
		}
	...
	private static String[] deviceTypeDescriptors = { "Cart",
			"System (or Host)" };
	private static String[] manufacturerCodeDescriptors = { "Unknown",
			"Burdick", "Cambridge", "Compumed",
			"Datamed",
			"Fukuda", // 5
			"Hewlett-Packard", "Marquette Electronics", "Mortara Instruments",
			"Nihon Kohden",
			"Okin", // 10
			"Quinton", "Siemens", "Spacelabs", "Telemed",
			"Hellige", // 15
			"ESA-OTE", "Schiller", "Picker-Schwarzer", "Elettronica-Trentina",
			"Zwönitz", // 20
	};
	...
	private class MachineIDField extends Field {
		...
		void getNamedcode(int code) {
			switch (code) {
			case 19:
				nameOfCode = "ISO8859_5";
				break;
			case 0:
				nameOfCode = "ASCII";
				break;
			case 3:
				nameOfCode = "ISO8859_2";
				break;
			case 11:
				nameOfCode = "ISO8859_4";
				break;
			case 27:
				nameOfCode = "ISO8859_6";
				break;
			case 35:
				nameOfCode = "ISO8859_7";
				break;
			case 43:
				nameOfCode = "ISO8859_8";
				break;
			case 51:
				nameOfCode = "ISO8859_11";
				break;
			default:
				nameOfCode = "ASCII";
				break;
		...
		void read() throws IOException {
			int remaining = length;
			if (remaining >= 36) { // variable length part for
									// analysingProgramRevisionNumber starts
									// after 0-35 bytes
				...
				modelDescription = new byte[6]; // 9-14 5 chars + null
												// terminator
				...
				getNamedcode(languageSupportCode);
				bytesRead++;
				sectionBytesRemaining--;
				remaining--;
				...
				if (analysingProgramRevisionNumberLength >= 1) {
					...
					i.readInsistently(analysingProgramRevisionNumber, 0,
							analysingProgramRevisionNumberLength);
			...
			if (remaining > 0) {
				...
				sectionBytesRemaining -= remaining;
				bytesRead += remaining;
				byte[] bt1 = new byte[remaining];
				byte[] bt2 = new byte[remaining];
				byte[] bt3 = new byte[remaining];
				byte[] bt4 = new byte[remaining];
				int k = 0;
				...
				setSerialnumber(new String(
						new String(bt1, nameOfCode).getBytes("UTF-8")));
				int j = 0;
				for (int i = k; i < remaining; i++) {
					if ((char) value[i] == '\0') {
						k = i + 1;
						break;
					} else
				...
				setVerPO(new String(
						new String(bt2, nameOfCode).getBytes("UTF-8")));
				j = 0;
				for (int i = k; i < remaining; i++) {
					if ((char) value[i] == '\0') {
						k = i + 1;
				...
				setPOSCP(new String(
						new String(bt3, nameOfCode).getBytes("UTF-8")));
				j = 0;
				for (int i = k; i < remaining; i++) {
					if ((char) value[i] == '\0') {
						k = i + 1;
				...
				setManufucture(new String(
						new String(bt4, nameOfCode).getBytes("UTF-8")));
		...
		String validate() {
			...
			if (deviceType >= deviceTypeDescriptors.length
					|| deviceTypeDescriptors[deviceType] == null) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": Unrecognized Device Type value " + deviceType
						+ " dec");
				strbuf.append("\n");
			}
			...
			if (manufacturerCode != 255) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": Use of legacy manufacturer code value, expected 255 dec, got "
						+ deviceType + " dec");
				strbuf.append("\n");
				if (manufacturerCode != 100) { // 100 is assigned to Other
					if (manufacturerCode >= manufacturerCodeDescriptors.length
							|| manufacturerCodeDescriptors[manufacturerCode] == null) {
						strbuf.append(describeFieldBriefly(tag));
						strbuf.append(": Unrecognized Device Type value "
								+ deviceType + " dec");
						strbuf.append("\n");
					}
				}
			...
			if (mainsFrequency >= mainsFrequencyDescriptors.length
					|| mainsFrequencyDescriptors[mainsFrequency] == null) {
				strbuf.append(describeFieldBriefly(tag));
				strbuf.append(": Unrecognized Mains Frequency value "
						+ deviceType + " dec");
				strbuf.append("\n");
			}

			{
				...
				if (highNibble < 9 || highNibble > 12) {
					strbuf.append(describeFieldBriefly(tag));
					strbuf.append(": Unrecognized Protocol Compatibility Level 0x"
							+ Integer.toHexString(protocolCompatibilityLevel));
					strbuf.append("\n");
				}
			}
		...
		protected int getValueLengthForPurposeOfValidation() {
			return analysingProgramRevisionNumberLength; // should subtract
															// count of any null
															// terminators
															// within or at the
															// end :(
		...
		public String getValueAsString() {
			...
			strbuf.append(modelDescription == null ? "" : new String(
					modelDescription));
			...
			strbuf.append(", capabilitiesCode=");
			strbuf.append(capabilitiesCode);
			...
			strbuf.append(analysingProgramRevisionNumber == null ? ""
					: new String(analysingProgramRevisionNumber));

			return strbuf.toString();
		}
	}

	private ArrayList[] fields; // (mostly sparse) array of 256 possible
								// ArrayLists of Field, indexed by tag
		...
		while (sectionBytesRemaining > 0) {
			...
			if (className != null) {
				if (className.equals("Text")) {
					...
					field = new ValueWithUnitsField(tag, length,
							ageUnitsDescription);
					...
					field = new ValueWithUnitsField(tag, length,
							weightUnitsDescription);
				} else if (className.equals("Sex")) {
					field = new SingleCodedValueField(tag, length,
							sexDescriptors);
					...
					field = new TwinCodedValueField(tag, length,
							electrodePlacement12LeadDescriptors,
							electrodePlacementXYZLeadDescriptors);
			...
			if (field.isTerminator()) {
				...
				if (sectionBytesRemaining > 1) {
					System.err
							.println("Section 1 Encountered terminator but more than one padding byte in section "
									+ sectionBytesRemaining
									+ " dec (0x"
									+ Long.toHexString(sectionBytesRemaining)
									+ ") bytes\n");
				}
			...
			if (fields[field.tag] == null) {
				fields[field.tag] = new ArrayList(); // lazy instantiation as we
														// encounter first
														// instance, since
														// sparse
			}
	...
	/**
	 * <p>
	 * Get the value of multiple instances of the same field (tag) as a single
	 * string.
	 * </p>
	 * 
	 * @param fieldName
	 ...
	 */
	public String getConcatenatedStringValuesOfAllOccurencesOfNamedField(
			String fieldName) {
		String s = null;
